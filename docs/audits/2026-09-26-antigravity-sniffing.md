...
| 13 | **Low** — README says v0.22.0; API table misses the new endpoints | ✅ real | **Fixed**: the version string is gone — replaced by a live badge row (release / build / downloads, shields.io) that cannot go stale; `/classify`, `/sniff/patterns`, `/sniff/rank` are in the table |
